...
class SearchBarController: UIViewController, UITextFieldDelegate
{
    ...
    private var _isTxtEditing:Bool = false     // 是否在编辑中
    
    var isTxtEditing: Bool {
        set {
            _isTxtEditing = newValue
            if _isTxtEditing {
    ...
    @IBAction func search() {
        if isTxtEditing == true {   // 编辑状态，按钮为：取消
            isTxtEditing = !isTxtEditing
            delegate?.clickBtn()
        } else {   // 非编辑状态，按钮为：附近
            delegate?.nearbyBtn?()
        }
...
//        return true
    ...
    func textFieldDidBeginEditing(textField: UITextField) {
        print("search bar textFieldDidBeginEditing called when added")
